...
+ 以下文代码为例，多态的意义在于对于一个可以以父类``Student()``实例为形参调用的函数``put_age()``，子类也可以作为形参调用，不用深究子类是派生出的何种类型，即只要来自于同一个父类``Student``，调用方只管调用，不管细节，而当我们新增一种子类时，只要确保原方法编写正确，不用管原来的代码是如何调用的，即“开闭原则”
	...
	```
	+ 开闭原则即对扩展开放(允许新增子类)；对修改封闭(不需要修改依赖父类的)
### 1.4.3 类的导入
...
## 1.5 文件处理
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDAxMzExMDE4LDYyMTM5OTAwMiwxNDY4NT
IyNTc5LDE3MzgwMjk3MzMsLTE2Mjg4NTY3MDMsLTk2MzUxMzA0
Nyw2MzA1NTY2MTQsLTc2NzU1NzUyNCwxMDg0NjYxNzE4LC0xND
