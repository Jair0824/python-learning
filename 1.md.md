...
+ 以下文代码为例，多态的意义在于对于一个可以以父类实例为形参调用的函数，子类也可以作为形参调用，不用深究子类是派生出的何种类型，即只要来自于同一个父类，调用方只管调用，不管细节，而当我们新增一种子类时，只要确保原方法编写正确，不用管原来的代码是如何调用的，即“开闭原则”
	```python
	def Student():
		--snip--
	
	def Un_Student(Student):
		--snip--
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTc2NzU1NzUyNCwxMDg0NjYxNzE4LC0xND
I5MzMxNzUxLC0yMDE4MzQ4NTg5LDMzNzE0NjE4NSwyNjQzOTY3
MzIsNjM4MjQxOTI2LDkxOTQwNzIyNCwxMTU3MTk1MDAsLTEwMD
E5MjcxMzEsLTExMDI3NTM3MTcsLTE1OTIzMzgzNSwtNjg2NzA1
MDgzLDEzNzc4MTI5OTYsNzA5OTMxMzUwLDEzNTEzNTM3NjIsLT
E5Mzc3Njk5ODgsLTI3MDE0MTM0NywxNzg2NjA1OTEzLDgzNjY4
ODMzN119
-->