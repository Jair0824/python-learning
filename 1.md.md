...
+ 使用``with``关键字打开文件
	...
	```
	关键字with在不需要访问文件后将其关闭。在调用``open()``和``close()``函数来打开和关闭文件时，如果程序存在bug，导致``close()``语句未执行，文件将不会关闭，可能会导致数据丢失或受损；如果在程序中过早地调用close()，则需要使用文件时已无法访问，会导致更多的错误。通过使用``with``关键字，Python可直接确定合适的时候自动将其关闭。
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg2MTA3MDc2LDE3Nzg0NTEwNjgsMTE5Mz
Q4NzA4MiwtMTgyNDQ0Nzg0LC02ODMzMTgwMzgsLTE3NjEwMTI0
NzEsLTUwNjc3MTcwNiw3MTgwMzk3NTUsLTIwOTQxMzEwMzYsMT
g5OTMyNTU2Niw5MzIxMjgyNzQsLTg0MDIzNzU0Niw2MjEzOTkw
MDIsMTQ2ODUyMjU3OSwxNzM4MDI5NzMzLC0xNjI4ODU2NzAzLC
05NjM1MTMwNDcsNjMwNTU2NjE0LC03Njc1NTc1MjQsMTA4NDY2
MTcxOF19
-->