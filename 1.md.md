...
+ 以上文``Student()``类为例，当需要创建一个新的与``Studeng()``类有类似方法的``Un_student()``类时，可使用继承。需注意在编写子类时文件中必须包含父类，需使用``super()``函数从``__init__()``方法初始化父类的属性
	...
	class Un_student(Student):
		def __init__(self,name,age,sex):
			...
			self.sex=sex
	```
+ 当创建一个子类时，可以为该子类创建新的不同于父类的方法


<!--stackedit_data:
eyJoaXN0b3J5IjpbMTc4Njc3Mzc1OSwxMTU3MTk1MDAsLTEwMD
...
ODMzNyw2MzE2MzY3MTYsLTc0OTkyOTM5MywxODAxOTY2MTY0LD
IyMjA0NDUxMCwtMTk1MjM5MjYyMCwzMjI5NTY1MiwtNTk1MDM5
ODk1XX0=
-->