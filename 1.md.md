...
## 1.7 测试
### 1.7.1  测试函数
Python的``unittest``模块提供了代码测试工具，测试有单元测试、测试用例、全覆盖式测试等形式；单元测试用于核实函数的某个方面没有问题；测试用例是一组单元测试，这些单元测试一起核实函数在各种情形下的行为都符合要求。全覆盖式测试用例包含一整套单元测试，涵盖了各种可能的函数使用方式


<!--stackedit_data:
eyJoaXN0b3J5IjpbLTYxNjQ5MzEwNCwyMTA3NTc5MTk1LDEzND
