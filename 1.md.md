...
+ 以下文代码为例，多态的意义在于对于一个可以以父类``Student()``实例为形参调用的函数``put_age()``，子类也可以作为形参调用，不用深究子类是派生出的何种类型，即只要来自于同一个父类``Student``，调用方只管调用，不管细节，而当我们新增一种子类时，只要确保原方法编写正确，不用管原来的代码是如何调用的，即“开闭原则”
	...
	```
### 1.4.3 类的导入
Python允许你将类存储在模块中，然后在主程序中导入所需的模块，导入类的方法包括多种，与函数模块化设计类似
+  导入单个类：在编写大规模程序时可以在单个


<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI4MjY2MjA1MywtOTYzNTEzMDQ3LDYzMD
U1NjYxNCwtNzY3NTU3NTI0LDEwODQ2NjE3MTgsLTE0MjkzMzE3
NTEsLTIwMTgzNDg1ODksMzM3MTQ2MTg1LDI2NDM5NjczMiw2Mz
gyNDE5MjYsOTE5NDA3MjI0LDExNTcxOTUwMCwtMTAwMTkyNzEz
MSwtMTEwMjc1MzcxNywtMTU5MjMzODM1LC02ODY3MDUwODMsMT
M3NzgxMjk5Niw3MDk5MzEzNTAsMTM1MTM1Mzc2MiwtMTkzNzc2
OTk4OF19
-->