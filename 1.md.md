...
+  导入单个类：在编写大规模程序时可以在一个模块中包含单个类，调用类中的方法时reng如在``student.py``文件中导入``Student``类并创建实例
	```python
	from student import Student

	S_1=Student("Peter",23) #创建实例并传入参数
	```
+ 同一模块存储多个类：在同一模块中可以存储任意数量的类，导入时仍使用``from -- import --``语句


<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMjIyNDQ4NTQsMTczODAyOTczMywtMT
