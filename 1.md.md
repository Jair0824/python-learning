...
+ 在一个模块中导入另一个模块：有时候，需要将类分散到多个模块中，以免模块太大，或在同一个模块中存储不相关的类。将类存储在多个模块中时，可能会存在一个模块中的类依赖于另一个模块中的类的情况。在这种情况下，可在前一个模块中导入必要的类。如若将``Student``存储在``s_1.py``模块中，将 ``Un_Student``、``PhD``存储在``s_2.py``模块中，由于``Un_Student``、``PhD``依赖于


<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4NDAzMTA2ODgsMTczODAyOTczMywtMT
YyODg1NjcwMywtOTYzNTEzMDQ3LDYzMDU1NjYxNCwtNzY3NTU3
NTI0LDEwODQ2NjE3MTgsLTE0MjkzMzE3NTEsLTIwMTgzNDg1OD
ksMzM3MTQ2MTg1LDI2NDM5NjczMiw2MzgyNDE5MjYsOTE5NDA3
MjI0LDExNTcxOTUwMCwtMTAwMTkyNzEzMSwtMTEwMjc1MzcxNy
wtMTU5MjMzODM1LC02ODY3MDUwODMsMTM3NzgxMjk5Niw3MDk5
MzEzNTBdfQ==
-->