...
+ 多态的基础是相同的数据类型，即从同一类父类发源；当在面向对象编程中定义一个``class``时即定义了一个数据类型，判断一个变量是否是某个类型可以用``isinstance()``函数进行判断；同时子类的数据类型可视作与父类相同，反之不可以
	```python
	class Student():
		--snip--
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY5MzA4MjgzMCwtMTQyOTMzMTc1MSwtMj
