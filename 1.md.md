...
+  导入单个类：在编写大规模程序时可以在一个模块中包含单个类，调用类中的方法时仍使用句点语法，如在只有一个模块的``student.py``文件中导入``Student``类并创建实例
	...
	S_1.out()
	```
+ 同一模块存储多个类：在同一模块中可以存储任意数量的类，导入时仍使用``from -- import --``语句，调用其中的方法时仍使用句点语法，需指定模块名，如包含了``Un_Student``和``PhD``类的``student1.py``文件
	```python
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTM4NzIwMzczLDE3MzgwMjk3MzMsLTE2Mj
g4NTY3MDMsLTk2MzUxMzA0Nyw2MzA1NTY2MTQsLTc2NzU1NzUy
NCwxMDg0NjYxNzE4LC0xNDI5MzMxNzUxLC0yMDE4MzQ4NTg5LD
MzNzE0NjE4NSwyNjQzOTY3MzIsNjM4MjQxOTI2LDkxOTQwNzIy
NCwxMTU3MTk1MDAsLTEwMDE5MjcxMzEsLTExMDI3NTM3MTcsLT
