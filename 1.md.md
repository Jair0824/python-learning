...
+  导入单个类：在编写大规模程序时可以在一个模块中包含单个类，调用类中的方法时仍使用句点语法，如在``student.py``文件中导入``Student``类并创建实例
	```python
	...
	S_1.out()
	```
+ 同一模块存储多个类：在同一模块中可以存储任意数量的类，导入时仍使用``from -- import --``语句，调用其中的方法时仍使用句点语法，需指定


<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg5NDIxODQ1NywxNzM4MDI5NzMzLC0xNj
I4ODU2NzAzLC05NjM1MTMwNDcsNjMwNTU2NjE0LC03Njc1NTc1
MjQsMTA4NDY2MTcxOCwtMTQyOTMzMTc1MSwtMjAxODM0ODU4OS
wzMzcxNDYxODUsMjY0Mzk2NzMyLDYzODI0MTkyNiw5MTk0MDcy
MjQsMTE1NzE5NTAwLC0xMDAxOTI3MTMxLC0xMTAyNzUzNzE3LC
0xNTkyMzM4MzUsLTY4NjcwNTA4MywxMzc3ODEyOTk2LDcwOTkz
MTM1MF19
-->