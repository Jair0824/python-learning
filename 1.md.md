...
## 1.7 测试
### 1.7.1  测试函数
+ Python的``unittest``模块提供了代码测试工具，测试有单元测试、测试用例、全覆盖式测试等形式；单元测试用于核实函数的某个方面没有问题；测试用例是一组单元测试，这些单元测试一起核实函数在各种情形下的行为都符合要求。全覆盖式测试用例包含一整套单元测试，涵盖了各种可能的函数使用方式，以对``calc.py``文件中的``calc()``函数进行测试为例
	```python
	#calc.py
	def calc(a,b):
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ3ODQwODU0NiwyMTA3NTc5MTk1LDEzND
U1MTE2MTEsLTE2MTgwNjAzMjAsOTg2NTM3NDAyLC0xNDEzOTM1
NjEwLDE2MTc5NjA2NjMsLTc0MzQ2MDk1LC0xMjM5Njg5MjkzLC
03OTgwODAwMjgsMTk0NzI0Mzg0NSwxODA2MDM5MTA5LDE1Nzg4
NjYzNzUsMTQyNTg3NzMxMiwxNjA3NTA0MTcwLDg1OTczNjAxNy
wxNzc4NDUxMDY4LDExOTM0ODcwODIsLTE4MjQ0NDc4NCwtNjgz
MzE4MDM4XX0=
-->