...
## 1.7 测试
### 1.7.1  测试函数
+ Python的``unittest``模块提供了代码测试工具，测试有单元测试、测试用例、全覆盖式测试等形式；单元测试用于核实函数的某个方面没有问题；测试用例是一组单元测试，这些单元测试一起核实函数在各种情形下的行为都符合要求。全覆盖式测试用例包含一整套单元测试，涵盖了各种可能的函数使用方式，以对以下函数进行测试为例
	```python
	def calc(a,b):
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ5MzkxOTg3LDIxMDc1NzkxOTUsMTM0NT
UxMTYxMSwtMTYxODA2MDMyMCw5ODY1Mzc0MDIsLTE0MTM5MzU2
MTAsMTYxNzk2MDY2MywtNzQzNDYwOTUsLTEyMzk2ODkyOTMsLT
c5ODA4MDAyOCwxOTQ3MjQzODQ1LDE4MDYwMzkxMDksMTU3ODg2
NjM3NSwxNDI1ODc3MzEyLDE2MDc1MDQxNzAsODU5NzM2MDE3LD
E3Nzg0NTEwNjgsMTE5MzQ4NzA4MiwtMTgyNDQ0Nzg0LC02ODMz
MTgwMzhdfQ==
-->