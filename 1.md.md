...
+ 以下文代码为例，多态的意义在于对于一个可以以父类``Student()``实例为形参调用的函数``put_age()``，子类也可以作为形参调用，不用深究子类是派生出的何种类型，即只要来自于同一个父类``Student``，调用方只管调用，不管细节，而当我们新增一种子类时，只要确保原方法编写正确，不用管原来的代码是如何调用的，即“开闭原则”
	...
	```
### 1.4.3 类的导入
+ Python允许你将类存储在模块中，然后在主程序中导入所需的模块，导入的方法包括多种
	


<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2ODQyMjEwODcsNjMwNTU2NjE0LC03Nj
c1NTc1MjQsMTA4NDY2MTcxOCwtMTQyOTMzMTc1MSwtMjAxODM0
ODU4OSwzMzcxNDYxODUsMjY0Mzk2NzMyLDYzODI0MTkyNiw5MT
k0MDcyMjQsMTE1NzE5NTAwLC0xMDAxOTI3MTMxLC0xMTAyNzUz
NzE3LC0xNTkyMzM4MzUsLTY4NjcwNTA4MywxMzc3ODEyOTk2LD
cwOTkzMTM1MCwxMzUxMzUzNzYyLC0xOTM3NzY5OTg4LC0yNzAx
NDEzNDddfQ==
-->