...
#### -类的创建
+ 定义类使用``class``关键字，以创建``Student``类为例，定义类后即可根据该类创建实例，创建方法为``类名(参数1，参数2,...)``
  ```python
  class Student():
  ...
  ```
  + ``class``后为类名，通常首字母大写
  + ``__init__()``为一种方法，即类中的函数，用于初始化对象的属性；在Python前后中加``__``的方法名为Python默认方法；在类中还可以定义普通方法，方法名前不需要加``__``，
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI0MzMxMjY3NSwtMjcwMTQxMzQ3LDE3OD
Y2MDU5MTMsODM2Njg4MzM3LDYzMTYzNjcxNiwtNzQ5OTI5Mzkz
LDE4MDE5NjYxNjQsMjIyMDQ0NTEwLC0xOTUyMzkyNjIwLDMyMj
k1NjUyLC01OTUwMzk4OTUsLTUxNDEyMjUxNSw2ODEzNzE2Mjcs
MTM4NTc4ODY2OCwxOTEzNTM2NTYxLC0zMzAzMDk0ODgsLTM2Nz
Q2OTc5Myw3Mjg2MjIzNzYsMzg5MzIyOTY0LC0xNjM5MTUwMDYw
XX0=
-->