...
```
#### - ``setUp()``方法
在前面的``test_information.py``中，在每个测试方法中都创建了一个``test_student``实例，并在每个方法中都创建了答案。``unittest.TestCase``类包含方法setUp()，让我们只需创建这些对象一次，并在每个测试方法中使用它们。如果你在TestCase类中包含了方法setUp()，Python将先运行它，再运行各个以test_打头的方法。这样，在你编写的每个测试方法中都可使用在方法setUp()中创建的对象了。

<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI1MjQ5NjA0MiwyNjMzMDc1NjUsLTExMj
QyNDIwODgsLTEyNzk4MDg0OTksMTA4MzAzMTMyNywxNTM0NjIw
NzY0LC04MDM2NDI4MzIsLTE1MjE2NTMzOSwtNTQ3NDc2NjkyLC
0xODY4MDAyMjk0LC0yODE5MjUzODYsNDM5NjAwMTAxLC0xOTMy
MTQ0OTk4LC0xNDE0NTYwNDc1LDkyNDc3NDk0MywxNDYwNjYxNT
Y4LDU1MjM4NzE4NiwxMjIzMTY1MzQyLDE0ODQ3OTg3OTYsMjEw
NzU3OTE5NV19
-->