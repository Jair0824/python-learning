...
+ 当创建一个子类时，可以为该子类创建新的不同于父类的属性和方法，仍以``Student()``类为例，可在创建子类``Un_student()``时为其添加新的属性和方法，根据子类创建的实例都将包含新的属性和方法，而根据父类创建的实例则没有
	...
	```
+ 当父类中的方法不符合子类的要求时可以进行重写，形成一个与父类中的某个方法在名称、形参列表和返回类型上都相同，
但方法体不同+子类还可以重新定义从父类继承的方法，称为覆盖 (overriding)。覆盖的规则是子
类中定义的某个方法和父类中的某个方法在名称、形参列表和返回类型上都相同，
但方法体不同。覆盖体现了子类和父类在功能上的差异


<!--stackedit_data:
eyJoaXN0b3J5IjpbNzM3NTc4MzI2LDkxOTQwNzIyNCwxMTU3MT
k1MDAsLTEwMDE5MjcxMzEsLTExMDI3NTM3MTcsLTE1OTIzMzgz
NSwtNjg2NzA1MDgzLDEzNzc4MTI5OTYsNzA5OTMxMzUwLDEzNT
EzNTM3NjIsLTE5Mzc3Njk5ODgsLTI3MDE0MTM0NywxNzg2NjA1
OTEzLDgzNjY4ODMzNyw2MzE2MzY3MTYsLTc0OTkyOTM5MywxOD
AxOTY2MTY0LDIyMjA0NDUxMCwtMTk1MjM5MjYyMCwzMjI5NTY1
Ml19
-->